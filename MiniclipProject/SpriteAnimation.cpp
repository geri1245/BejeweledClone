...
void SpriteAnimation::Draw(Vec2 location, int frameSize, double progress)
{
    ...
    _screen->DrawTexture(location, _textures[indexToDraw], nullptr, &dstRect);
}
...
void SpriteAnimation::LoadAssets(const std::string& animationDirectory)
{
    ...
    for (auto const& dirEntry : std::filesystem::directory_iterator { assetPath }) {
        ...
        textures.push_back({ dirEntry.path().filename().string(), texture });
    }
    ...
    std::transform(textures.begin(), textures.end(), std::back_inserter(_textures), [](auto pair) { return pair.second; });
}
